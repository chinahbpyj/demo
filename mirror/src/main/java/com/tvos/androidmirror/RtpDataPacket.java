...
public class RtpDataPacket {
    ...
    public static RtpDataPacket decode(byte[] data) {
        return decode(ChannelBuffers.wrappedBuffer(data));
    }

    public static RtpDataPacket decode(ChannelBuffer buffer) throws IndexOutOfBoundsException {
        if (buffer.readableBytes() < 12) {
            throw new IllegalArgumentException("A RTP packet must be at least 12 octets long");
        }

        // Version, Padding, eXtension, CSRC Count
        RtpDataPacket packet = new RtpDataPacket();
    ...
    public static ChannelBuffer encode(int fixedBlockSize, RtpDataPacket packet) {
